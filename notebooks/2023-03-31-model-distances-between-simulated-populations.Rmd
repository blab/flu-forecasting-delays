...
```{r}
additional.distance.to.future <- (exp(post.samples$a + post.samples$b_h) - exp(post.samples$a)) * 12
...
```{r}
additional.distance.to.present <- (exp(post.samples$a + post.samples$b_s) - exp(post.samples$a)) * 12
...
hist(additional.distance.to.present, xlab="rate of distance to the present (AAs/year)")
...
ggsave("reduction-in-distance-to-future-by-scenario-for-simulated-populations.png")
```
