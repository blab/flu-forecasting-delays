...
```{r}
distances <- read_tsv("results/model_inputs_for_simulated_populations.tsv")
```
...
Fit the same model but with centered predictors, to see how the fit differs.
...
Plot the centered model coefficients and HPDI.
...
write_csv(as.data.frame(post.samples.centered), "results/simulated_posterior_centered.csv")
```
...
Plot the relationship between different forecast horizons and distance to the future.
...
fixed.delay <- min(distances$delay_s)
...
```{r}
plot(delay.seq, mu.mean.for.delays, ylim=c(0, 7))
...
```{r}
fixed.horizon <- max(horizon.seq)
unique.delays_s <- unique(distances$delay_s)
scenario.2.delays <- c(unique.delays_s[2], unique.delays_s[3])
...
scenario.3.params <- rbind(
  list(horizon=horizon.seq[2], delay=unique.delays_s[2]),
  list(horizon=horizon.seq[4], delay=unique.delays_s[3])
)
...
```{r}
horizons <- unique(distances$horizon_s)
delays <- unique(distances$delay_s)
horizons_delays <- expand.grid(delay_s=delays, horizon_s=horizons)
post <- sim(simple.model.standardized, horizons_delays)
```
