...
Despite continued methodological improvements to long-term forecasting models, these constraints of a 12-month forecast horizon and 3-month average submission lags impose an upper bound on any model's accuracy.
The global response to the SARS-CoV-2 pandemic revealed that the adoption of modern vaccine technology like mRNA vaccines can reduce how far we need to forecast into the future to 6 months or less and that expanded support for sequencing can reduce submission lags to GISAID to 1 month on average.
To determine whether these public health policy changes could improve long-term forecasts for seasonal influenza, we quantified the effects of reducing forecast horizons and submission lags on the accuracy of forecasts for A/H3N2 populations.
...
These results show the potential to substantially improve the accuracy of existing influenza forecasting models through the public health policy changes of modernizing influenza vaccine development and increasing global sequencing capacity.
\end{abstract}
...
The dominant influenza vaccine platform is an inactivated whole virus vaccine grown in chicken eggs \citep{Wong2013} which takes 6 to 8 months to develop, contains a single representative vaccine virus per seasonal influenza subtype including A/H1N1pdm, A/H3N2, and B/Victoria \citep{Morris2018}, and for which only the HA protein content is standardized \citep{Yamayoshi2019}.
These constraints require the WHO to select a single virus per subtype that is immunologically-representative of the next season's dominant HA approximately 12 months before the peak of that next season.
...
For this reason, the decision process is partially informed by computational models that attempt to predict the genetic composition of seasonal influenza populations 12 months in the future \citep{Morris2018}.
...
Recent models include phenotypic data from serological experiments \citep{Morris2018,Huddleston2020,Meijers2023,Meijers2025}.
Since most serological experiments occur after genetic sequencing \citep{Hampson2017} and all forecasting models depend on HA sequences to determine the viruses circulating at the time of a forecast, sequence availability is the initial limiting factor for any influenza forecasts.
...
Work on mRNA vaccines for influenza viruses dates back over a decade \citep{Petsch2012,Brazzoli2016,Pardi2018,Feldman2019}, and multiple vaccines have completed phase 3 trials by early 2025 \citep{Soens2025,NCT05540522}.
...
\begin{figure}[htb!]
\includegraphics[width=\linewidth]{distribution_of_delays_and_horizons}
\caption{Model of forecast horizons and submission lags.
  ...
  For each forecast horizon, we calculated the accuracy of forecasts under each of the three submission lags described below including no lag, realistic lag, and ideal lag.
...
{\includegraphics[width=6cm]{distribution_of_delays_by_pandemic_era}}\label{figsupp:distribution_of_delays_by_pandemic_era}
%
...
{\includegraphics[width=6cm]{h3n2_distribution_of_sequences_per_timepoint_and_delay}}\label{figsupp:distribution_of_h3n2_sequences_per_timepoint_and_delay}
%
...
{\includegraphics[width=6cm]{simulated_distribution_of_sequences_per_timepoint_and_delay}}\label{figsupp:distribution_of_simulated_sequences_per_timepoint_and_delay}
%
\figsupp[Number of all available sequences per region and year and proportion of sequences sampled by two different subsampling methods]
{
  A) Number of all available sequences per region and year in the GISAID EpiFlu database for the study period between April 1, 2005 and October 1, 2019.
  B) Proportion of sequences sampled per region and year with even subsampling across regions and year/month combinations at 90 viruses per month.
  C) Proportion of sequences sampled per region and year with even subsampling across regions and year/month combinations at 270 viruses per month.
}
{\includegraphics[width=6cm]{sequences_per_region}}\label{figsupp:sequences_per_region}
%
\figdata{Distribution of lags between sample collection and sequence submission in prepandemic and pandemic eras; see \texttt{distribution\_of\_submission\_lags.csv} at \url{https://doi.org/10.5281/zenodo.17259448}.}\label{figdata:distribution_of_submission_lags}
\end{figure}
...
\begin{figure}[htb]
\includegraphics[width=\linewidth]{h3n2_distances_to_the_future_by_delay_and_horizon}
...
{Distance to the future per timepoint (AAs) for simulated A/H3N2-like populations by forecast horizon and submission lag type based on forecasts from the ``true fitness'' model.
  \figsuppdata{Distance to the future for simulated A/H3N2-like populations; see \texttt{simulated\_distances\_to\_the\_future.csv} at \url{https://doi.org/10.5281/zenodo.17259448}.}}
{\includegraphics[width=6cm]{simulated_distances_to_the_future_by_delay_and_horizon}}\label{figsupp:simulated_distances_to_the_future}
%
...
{\includegraphics[width=6cm]{h3n2_optimal_distances_to_the_future_by_delay_and_horizon}}\label{figsupp:h3n2_optimal_distances_to_the_future}
%
...
{\includegraphics[width=6cm]{simulated_optimal_distances_to_the_future_by_delay_and_horizon}}\label{figsupp:simulated_optimal_distances_to_the_future}
%
\figdata{Distance to the future for natural A/H3N2 populations; see \texttt{h3n2\_distances\_to\_the\_future.csv} at \url{https://doi.org/10.5281/zenodo.17259448}.}\label{figdata:h3n2_distances_to_the_future}
\figsrccode{Jupyter notebook used to produce this figure and the supplemental figure: \texttt{workflow/notebooks/plot-distances-to-the-future-by-delay-type-and-horizon-for-population.py.ipynb}.}\label{figsrccode:distances_to_the_future}
\end{figure}

\begin{table}[htb]
  \begin{center}
    \input{h3n2_distances_to_the_future_by_delay_and_horizon.tex}
...
\begin{figure}[htb!]
\includegraphics[width=\linewidth]{h3n2_current_frequency_errors_by_delay}
...
{Clade frequency errors between simulated A/H3N2-like HA populations with ideal or realistic submission lags and populations without any submission lag.
  \figsuppdata{Current and future clade frequencies for simulated A/H3N2-like populations by forecast horizon and submission lag type; see \texttt{simulated\_clade\_frequencies.csv} at \url{https://doi.org/10.5281/zenodo.17259448}.}}
{\includegraphics[width=6cm]{simulated_current_frequency_errors_by_delay}}\label{figsupp:simulated_current_clade_frequency_errors}
%
\figdata{Current and future clade frequencies for natural A/H3N2 populations by forecast horizon and submission lag type; see \texttt{h3n2\_clade\_frequencies.csv} at \url{https://doi.org/10.5281/zenodo.17259448}.}\label{figdata:h3n2_clade_frequencies}
\figsrccode{Jupyter notebook used to produce this figure and the supplemental figure: \texttt{workflow/notebooks/plot-current-clade-frequency-errors-by-delay-type-for-populations.py.ipynb}.}\label{figsrccode:current_clade_frequency_errors}
\end{figure}
...
\begin{figure}[htb!]
\includegraphics[width=\linewidth]{h3n2_absolute_forecast_frequency_errors_by_delay_and_horizon}
...
{\includegraphics[width=6cm]{simulated_absolute_forecast_frequency_errors_by_delay_and_horizon}}\label{figsupp:simulated_absolute_forecast_clade_frequency_errors}
%
...
{\includegraphics[width=6cm]{h3n2_forecast_frequency_errors_by_delay_and_horizon}}\label{figsupp:h3n2_forecast_clade_frequency_errors}
%
...
{\includegraphics[width=6cm]{simulated_forecast_frequency_errors_by_delay_and_horizon}}\label{figsupp:simulated_forecast_clade_frequency_errors}
%
\figsrccode{Jupyter notebook used to produce this figure and the supplemental figures: \texttt{workflow/notebooks/plot-forecast-clade-frequency-errors-by-delay-type-and-horizon-for-population.py.ipynb}.}\label{figsrccode:forecast_clade_frequency_errors}
\end{figure}
...
\begin{table}[htb]
  \begin{center}
    \input{h3n2_forecast_frequency_errors_future_by_delay_and_horizon.tex}
...
\begin{figure}[htb!]
\includegraphics[width=\linewidth]{h3n2_effects_of_realistic_interventions}
...
{\includegraphics[width=6cm]{h3n2_total_absolute_forecast_frequency_errors_by_delay_and_horizon}}\label{figsupp:h3n2_total_absolute_clade_frequency_errors}
%
...
{Improvement of clade frequency errors for simulated A/H3N2-like populations between the status quo and realistic interventions.
  \figsuppdata{Differences in total absolute clade frequency error per future timepoint and clade between the status quo and realistic interventions for simulated A/H3N2-like populations; see \texttt{simulated\_effects\_of\_realistic\_interventions.csv} at \url{https://doi.org/10.5281/zenodo.17259448}.}}
{\includegraphics[width=6cm]{simulated_effects_of_realistic_interventions}}\label{figsupp:simulated_effects_of_realistic_interventions}
%
...
{\includegraphics[width=6cm]{simulated_total_absolute_forecast_frequency_errors_by_delay_and_horizon}}\label{figsupp:simulated_total_absolute_clade_frequency_errors}
%
...
{Improvement of distances to the future (AAs) for A/H3N2 populations between the status quo (12-month forecast horizon and realistic submission lags) and realistic interventions.\
  ...
  \figsuppdata{Improvement of distances to the future per future timepoint for A/H3N2 populations; see \texttt{h3n2\_effects\_of\_realistic\_interventions\_on\_distances\_to\_the\_future.csv} at \url{https://doi.org/10.5281/zenodo.17259448}.}}
{\includegraphics[width=6cm]{h3n2_effects_of_realistic_interventions_on_distances_to_the_future}}\label{figsupp:h3n2_effects_of_realistic_interventions_on_distances_to_the_future}
%
...
{Improvement of distances to the future (AAs) for simulated A/H3N2-like populations between the status quo (12-month forecast horizon and realistic submission lags) and realistic interventions.\
  The effects of interventions are the differences between distances to the future per future timepoint under the status quo and specific interventions.
  \figsuppdata{Improvement of distances to the future per future timepoint for simulated A/H3N2-like populations; see \texttt{simulated\_effects\_of\_realistic\_interventions\_on\_distances\_to\_the\_future.csv} at \url{https://doi.org/10.5281/zenodo.17259448}.}}
{\includegraphics[width=6cm]{simulated_effects_of_realistic_interventions_on_distances_to_the_future}}\label{figsupp:simulated_effects_of_realistic_interventions_on_distances_to_the_future}
%
\figdata{Differences in total absolute clade frequency error per future timepoint and clade between the status quo and realistic interventions for A/H3N2 populations; see \texttt{h3n2\_effects\_of\_realistic\_interventions.csv} at \url{https://doi.org/10.5281/zenodo.17259448}.}\label{figdata:h3n2_effects_of_realistic_interventions}
...
\figsrccode{Jupyter notebook used to produce effects of interventions on distances to the future: \texttt{workflow/notebooks/plot-distances-to-the-future-by-delay-type-and-horizon-for-population.py.ipynb}.}\label{figsrccode:effects_of_realistic_interventions_on_distances_to_the_future}
\end{figure}

\begin{table}[htb]
  \begin{center}
    \input{h3n2_effects_of_realistic_interventions.tex}
...
To verify the robustness of these results, we replicated our entire analysis of A/H3N2 populations using a subsampling scheme that tripled the number of viruses selected per month from 90 to 270 (\FIGSUPP[model_of_delays_and_horizons]{sequences_per_region}C).
We found the same pattern with this replication analysis, with average improvements of 0.93 AAs for the vaccine intervention, 0.21 AAs for the surveillance intervention, and 1.14 AAs for both interventions (\FIGSUPP[h3n2_optimal_effects_of_realistic_interventions]{h3n2_high_density_optimal_effects_of_realistic_interventions_on_distances_to_the_future}).
These effects of realistic interventions appeared consistent across the range of genetic diversity at future timepoints (\FIGSUPP[h3n2_optimal_effects_of_realistic_interventions]{h3n2_optimal_effects_of_realistic_interventions_on_distances_to_the_future_by_future_clade_entropy}) and for future seasons occurring in both Northern and Southern Hemispheres (\FIGSUPP[h3n2_optimal_effects_of_realistic_interventions]{h3n2_optimal_effects_of_realistic_interventions_on_distances_to_the_future_by_hemisphere}).
We noted a slightly greater median improvement in forecast accuracy associated with both improved vaccine interventions for the Southern Hemisphere seasons (1.03 and 1.42 AAs) compared to the Northern Hemisphere seasons (0.74 and 0.93 AAs).
...
\begin{figure}[htb!]
\includegraphics[width=\linewidth]{h3n2_optimal_effects_of_realistic_interventions_on_distances_to_the_future}
...
{Improvement of optimal distances to the future (AAs) for simulated A/H3N2-like populations between the status quo (12-month forecast horizon and realistic submission lags) and realistic interventions.\
  \figsuppdata{Improvement of optimal distances to the future per future timepoint for simulated A/H3N2-like populations; see \texttt{simulated\_optimal\_effects\_of\_realistic\_interventions\_on\_distances\_to\_the\_future.csv} at \url{https://doi.org/10.5281/zenodo.17259448}.}}
{\includegraphics[width=6cm]{simulated_optimal_effects_of_realistic_interventions_on_distances_to_the_future}}\label{figsupp:simulated_optimal_effects_of_realistic_interventions_on_distances_to_the_future}
%
\figsupp[Improvement of optimal distances to the future (AAs) for A/H3N2 populations sampled with higher density sampling (270 viruses per month).]
{Improvement of optimal distances to the future (AAs) for A/H3N2 populations between the status quo (12-month forecast horizon and realistic submission lags) and realistic interventions using forecasts based on sampling 270 viruses per month instead of the 90 viruses-per-month sampling used in the main results.
  \figsuppdata{Improvement of optimal distances to the future per future timepoint for A/H3N2 populations with higher density sampling; see \texttt{h3n2\_high\_density\_optimal\_effects\_of\_realistic\_interventions\_on\_distances\_to\_the\_future.csv} at \url{https://doi.org/10.5281/zenodo.17259448}.}}
{\includegraphics[width=6cm]{h3n2_high_density_optimal_effects_of_realistic_interventions_on_distances_to_the_future}}\label{figsupp:h3n2_high_density_optimal_effects_of_realistic_interventions_on_distances_to_the_future}
%
\figsupp[Improvement of optimal distances to the future (AAs) for A/H3N2 populations compared to the Shannon entropy of clade frequencies at the future timepoint being forecast to.]
{Improvement of optimal distances to the future (AAs) for A/H3N2 populations compared to the Shannon entropy of clade frequencies (estimated without submission lags) at the future timepoint being forecast to. Panel titles include the Pearson $r$ value between the improvement in distance and the future clade entropy.
  \figsuppdata{Improvement of optimal distances to the future for A/H3N2 populations compared to the Shannon entropy of clade frequencies at the future timepoint; see \texttt{h3n2\_optimal\_effects\_of\_realistic\_interventions\_on\_distances\_to\_the\_future\_by\_future\_clade\_entropy.csv} at \url{https://doi.org/10.5281/zenodo.17259448}.}}
{\includegraphics[width=6cm]{h3n2_optimal_effects_of_realistic_interventions_on_distances_to_the_future_by_future_clade_entropy}}\label{figsupp:h3n2_optimal_effects_of_realistic_interventions_on_distances_to_the_future_by_future_clade_entropy}
%
\figsupp[Improvement of optimal distances to the future (AAs) for A/H3N2 populations by intervention and the hemisphere with an active season during the future timepoint being predicted.]
{Improvement of optimal distances to the future (AAs) for A/H3N2 populations by intervention and the hemisphere with an active season during the future timepoint being predicted. We labeled future timepoints that occurred in October or January as ``Northern'' and ``Southern'' if the dates were in April or July.}
{\includegraphics[width=6cm]{h3n2_optimal_effects_of_realistic_interventions_on_distances_to_the_future_by_hemisphere}}\label{figsupp:h3n2_optimal_effects_of_realistic_interventions_on_distances_to_the_future_by_hemisphere}
%
\figdata{Differences in optimal distances to the future per future timepoint between the status quo and realistic interventions for A/H3N2 populations; see \texttt{h3n2\_optimal\_effects\_of\_realistic\_interventions\_on\_distances\_to\_the\_future.csv} at \url{https://doi.org/10.5281/zenodo.17259448}.}\label{figdata:h3n2_optimal_effects_of_realistic_interventions}
%
\figsrccode{Python notebook used to produce optimal effects of interventions on distances to the future: \texttt{workflow/notebooks/plot-distances-to-the-future-by-delay-type-and-horizon-for-population.py.ipynb}.}\label{figsrccode:optimal_effects_of_realistic_interventions_on_distances_to_the_future}
\figsrccode{Python notebook used to plot optimal effects by future clade entropy: \texttt{workflow/notebooks/plot-optimal-effects-of-interventions-by-clade-entropy.py}.}\label{figsrccode:optimal_effects_of_realistic_interventions_by_future_clade_entropy}
\figsrccode{Python notebook used to plot optimal effects by hemisphere: \texttt{workflow/notebooks/plot-optimal-effects-of-interventions-by-hemisphere.py}.}\label{figsrccode:optimal_effects_of_realistic_interventions_by_hemisphere}
\end{figure}
...
For example, virus samples from North America and Europe are overrepresented in the GISAID EpiFlu database, while samples from Africa and Asia are underrepresented (\FIGSUPP[model_of_delays_and_horizons]{sequences_per_region}).
As new H3N2 epidemics often originate from East and Southeast Asia and burn out in North America and Europe \citep{Bedford2015}, models that do not account for this geographic bias are more likely to incorrectly predict the success of lower fitness variants circulating in overrepresented regions and miss higher fitness variants emerging from underrepresented regions.
Additionally, the number of H3N2 HA sequences per year in the GISAID EpiFlu database has increased consistently since 2010, creating a temporal bias where any given season a model forecasts to will have more sequences available than the season from which forecasts occur.
The model we used in this study does not explicitly account for geographic variability of viral fitness and relies on time-scaled phylogenetic trees which can be computationally costly to infer for large sample sizes.
As a result, we needed to evenly sample the diversity of currently circulating viruses to produce unbiased trees in a reasonable amount of time.
Models that could estimate viral fitness per geographic region without inferring trees could use more available sequence data and reduce the uncertainty in current and future clade frequencies.
...
For example, virologists must choose potential vaccine candidates from the diversity of circulating clades months in advance of vaccine composition meetings to have time to grow virus in cells and eggs and measure antigenic drift with serological assays \citep{Morris2018,Loes2024}.
Earlier detection of viral sequences with important antigenic substitutions could determine whether corresponding vaccine candidates are available at the time of the vaccine selection meeting or not.
Newer methods to estimate influenza fitness use experimental measurements of viral escape from human sera \citep{Lee2019,Welsh2023,Meijers2025,Kikawa2025}, measurements of viral stability and cell entry \citep{Yu2025}, or sequences from neuraminidase, the other primary surface protein associated with antigenic drift \citep{Meijers2025}.
These methodological improvements all depend fundamentally on timely genomic surveillance efforts and the GISAID EpiFlu database to identify relevant influenza variants to include in their experiments.
...
Briefly, we selected 90 sequences per month, evenly sampling from major continental regions (Africa, Europe, North America, China, South Asia, Japan and Korea, Oceania, South America, Southeast Asia, and West Asia) and excluding sequences labeled as egg-passaged or missing complete date annotations.
For our forecasting analyses, we selected sequences collected between April 1, 2005 and October 1, 2019.
This sampling approach accounts for known regional biases in sequence availability through time \citep{McCarron2022} and makes inference of divergence and time trees computationally tractable.
This approach also exactly matches our previous study where we first trained the forecast models used in this study \citep{Huddleston2020}, allowing us to reuse those previously trained models.
With this subsampling approach, we selected between 7\% (Europe) and 91\% (Southeast Asia) of all available sequences per region across the entire study period with an average of 50\% and median of 52\% across all 10 regions (\FIGSUPP[model_of_delays_and_horizons]{sequences_per_region}).
To verify the reproducibility and robustness of our results, we reran the full forecasting analysis with a high-density subsampling scheme that selected 270 sequences per month with the same even sampling across regions and time as the original scheme.
With this approach, we selected between 17\% (Europe) and 97\% (Southeast Asia) of all available sequences per region with an average of 72\% sampled and a median of 83\% (\FIGSUPP[model_of_delays_and_horizons]{sequences_per_region}C).
...
Supplemental data are available on Zenodo at DOI \href{https://doi.org/10.5281/zenodo.17259448}{10.5281/zenodo.17259448}.
