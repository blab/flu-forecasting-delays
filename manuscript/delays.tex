...
\caption{Model of forecast horizons and submission lags.
  ...
  For each forecast horizon, we calculate the accuracy of forecasts under each of the three submission lags reflected above including no lag (blue), realistic lag (green), and ideal lag (orange).
  ...
  A gamma distribution fit to the observed lag distribution with a similar mean and shape (green) represents a realistic submission lag that we sample from to assign ``submission dates'' to simulated and natural A/H3N2 populations.
...
Reducing submission lags had little effect on forecast errors for small clades.

\begin{figure}[htb!]
\includegraphics[width=\linewidth]{figures/h3n2_forecast_frequency_errors_by_delay_and_horizon}
\caption{Forecast clade frequency errors for natural A/H3N2 populations by forecast horizon in months and submission lag type (none, ideal, or observed) for A) small clades ($<$10\% initial frequency) and B) large clades ($\ge$10\% initial frequency).
}
...
For large clades, almost all decreases in forecast horizon and submission lag (except ideal submission lags at the 12-month horizon) reduced the standard deviation of forecast errors and the absolute forecast errors (\FIG{h3n2_forecast_clade_frequency_errors}B, \TABLE{h3n2_forecast_clade_frequency_errors}).
Reducing the forecast horizon had a greater effect on the standard deviation and absolute values of forecast errors than reducing submission lags.
For example, the standard deviation of forecast errors at the 12-month horizon under realistic submission lags was 31\%, while the standard deviation for the 6-month horizon under realistic lags was 20\% (\TABLE{h3n2_forecast_clade_frequency_errors}).
Similarly, the average absolute forecast error decreased from 20\% at the 12-month horizon to 15\% at the 6-month horizon.
In contrast, the standard deviation at the 12-month horizon under ideal submission lags did not change from the realistic lags at 31\%, and the average absolute forecast error increased by 1\% from 20\%.
For all other forecast horizons, reducing the submission lags from realistic to ideal only reduced the standard deviation and the average absolute error by 1--2\%.
The strongest effect of reducing submission lags was the reduction in maximum forecast error, corresponding to reduction in underestimation of large clades.
...
These results confirmed that the effect size of submission lags on forecast errors is low relative to the effect of the forecast horizon.
However, reducing submission lags can substantially lower the upper bound for forecasting errors of large clades.
...
%
\figsupp[Improvement of optimal distances to the future (AAs) for A/H3N2 populations between the status quo (12-month forecast horizon and realistic submission lags) and realistic interventions.]
{Improvement of optimal distances to the future (AAs) for A/H3N2 populations between the status quo (12-month forecast horizon and realistic submission lags) and realistic interventions.\
...
We expect that decreasing forecast horizons and submission lags will have similar effect sizes in other forecasting models, too.
...
We could also start forecasting a month earlier than the current date to minimize the effect of submission lags on our estimates of the current global influenza population.
