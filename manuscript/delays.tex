...
% TB: vaccine is often available ~6 months later in August, even if the main target is February
...
However, the average delay between collection of clinical sample and the submission of its sequence to the Global Initiative on Sharing All Influenza Data (GISAID) EpiFlu database is $\sim$3 months.
% TB: this is an important distinction! Sequences are not collected. Samples are collected. Phrasing made it sound like labs are sitting on data.
Submission delays complicate the already difficult 12-month forecasting problem by reducing understanding of current clade frequencies at the time of forecasting.
...
% TB: A perfect model with informed DMS data could theoretically pick out of sample mutations that will arise. In a perfect world you could imagine a forecasting model that predicts multiple years out.
...
Unfortunately, the average delay between collection of a seasonal influenza A/H3N2 HA sample and submission of its sequence had been approximately 3 months in the era prior to the SARS-CoV-2 pandemic (\FIG{model_of_delays_and_horizons}A).
% TB: I don't like "3 months". I'd suggest either to caveat as rounded via "~3 months" or provide appropriate precision, ie "3.0 months"
...
\caption{Model of forecast horizons and submission delays.
  ...
  A gamma distribution fit to the observed delay distribution with a similar mean and shape (green) represents a realistic submission delay that we sample from to assign ``submission dates'' to simulated and natural A/H3N2 populations.
  ...
  % TB: Update x-axis label of panel B to "Delay between sample collection and sequence submission (days)" or "Lag between sample collection and sequence submission (days)".
...
%
\figsupp[Number and proportion of A/H3N2 sequences available per timepoint and delay type]
{
  A) Number of A/H3N2 sequences available per timepoint and delay type.
  B) Proportion of all A/H3N2 sequences without delay per timepoint and delay type.
}
...
%
\figsupp[Number and proportion of simulated A/H3N2-like sequences available per timepoint and delay type]
{
  A) Number of simulated A/H3N2-like sequences available per timepoint and delay type.
  B) Proportion of all simulated A/H3N2-like sequences without delay per timepoint and delay type.
}
...
{Distance to the future per timepoint (AAs) for simulated A/H3N2-like populations by forecast horizon and submission delay type based on forecasts from the ``true fitness'' model.
...
{Optimal distance to the future per timepoint (AAs) for natural A/H3N2 populations by forecast horizon and submission delay type based on posthoc empirical fitness of the initial population.}
...
{Optimal distance to the future per timepoint (AAs) for simulated A/H3N2-like populations by forecast horizon and submission delay type based on posthoc empirical fitness of the initial population.}
...
\begin{table}[htb]
  \begin{center}
    ...
    \caption{Distance to the future in amino acids (mean +/- standard deviation AAs) by forecast horizon (in months) and submission delay for A/H3N2 populations.}
...
Across all clade frequencies, we found that errors in current clade frequencies for A/H3N2 appeared normally distributed with lower variance in the ideal delay scenario than under realistic delays (\FIG{h3n2_current_clade_frequency_errors}A and B).
...
These results show that reducing submission delays for natural A/H3N2 populations from a 3-month average to a 1-month average could reduce the bias toward underestimated large clade frequencies and reduce the standard deviation of all current clade frequency errors by 50\%.
...
{Clade frequency errors between simulated A/H3N2-like HA populations with ideal or realistic submission delays and populations without any submission delay.
  \figsuppdata{Current frequencies per sequence in each simulated A/H3N2-like tree of the forecast analysis by delay type (none, ideal, and realistic); see \url{https://doi.org/xxx}}}
...
%
\figdata{Current frequencies per sequence in each natural A/H3N2 tree of the forecast analysis by delay type (none, ideal, and observed).}\label{figdata:h3n2_tip_attributes}
...
As with natural A/H3N2 populations, we observed a slight bias in simulated populations under realistic delays toward underestimation of large clade frequencies with a median error of 2\%.
We also observed a similar reduction in standard deviation of current frequency errors for these simulated A/H3N2-like populations when switching from realistic to ideal submission delays.
...
The majority of forecast frequency errors appeared to be normally distributed (\FIG{h3n2_forecast_clade_frequency_errors}).
...
However, we observed a long tail of small clades with underestimated future frequencies at all forecast horizons, indicating that correctly predicting the growth of small clades remains more difficult than predicting their decline (\FIG{h3n2_forecast_clade_frequency_errors}A).
Reducing submission delays had little effect on forecast errors for small clades.

\begin{figure}[htb!]
\includegraphics[width=\linewidth]{figures/h3n2_forecast_frequency_errors_by_delay_and_horizon}
\caption{Forecast clade frequency errors for natural A/H3N2 populations by forecast horizon in months and submission delay type (none, ideal, or observed) for A) small clades ($<$10\% initial frequency) and B) large clades ($\ge$10\% initial frequency).
}
...
{Forecast clade frequency errors for simulated A/H3N2-like HA populations by forecast horizon in months and submission delay type (none, ideal, or realistic) for A) small clades ($<$10\% initial frequency) and B) large clades ($\ge$10\% initial frequency).
  \figsuppdata{Current, predicted future, and observed future clade frequencies per initial timepoint, forecast horizon, and submission delay type for simulated A/H3N2-like populations; see \url{https://doi.org/xxx}}}
...
%
\figdata{Current, predicted future, and observed future clade frequencies per initial timepoint, forecast horizon, and submission delay type for A/H3N2 populations.}\label{figdata:h3n2_clade_frequencies}
...
For large clades, almost all decreases in forecast horizon and submission delay (except ideal submission delays at the 12-month horizon) reduced the standard deviation of forecast errors and the absolute forecast errors (\FIG{h3n2_forecast_clade_frequency_errors}B, \TABLE{h3n2_forecast_clade_frequency_errors}).
...
For example, the standard deviation of forecast errors at the 12-month horizon under realistic submission delays was 31\%, while the standard deviation for the 6-month horizon under realistic delays was 20\% (\TABLE{h3n2_forecast_clade_frequency_errors}).
...
The switch from realistic to ideal delays at 12-, 9-, 6-, and 3-month horizons reduced the maximum forecast error by 4\%, 21\%, 22\%, and 14\%, respectively (\TABLE{h3n2_forecast_clade_frequency_errors}).
We observed the same general patterns in simulated populations (\FIGSUPP[h3n2_forecast_clade_frequency_errors]{simulated_forecast_clade_frequency_errors}).
...
\begin{table}[htb]
  \begin{center}
    ...
    \caption{Errors in clade frequencies between observed and predicted values by forecast horizon (in months) and submission delay for A/H3N2 clades with an initial frequency $\geq$10\% under the given delay scenario.}
...
For the same future timepoints present in the status quo conditions, we selected the corresponding forecasts for a 6-month horizon and a realistic delay, a 12-month horizon and an ideal delay, and 6-month horizon and an ideal delay.
...
\caption{Improvement of clade frequency errors for A/H3N2 populations between the status quo (12-month forecast horizon and realistic submission delays) and realistic interventions.}
...
%
\figsupp[Improvement of distances to the future (AAs) for A/H3N2 populations between the status quo (12-month forecast horizon and realistic submission delays) and realistic interventions.]
{Improvement of distances to the future (AAs) for A/H3N2 populations between the status quo (12-month forecast horizon and realistic submission delays) and realistic interventions.\
...
%
\figsupp[Improvement of distances to the future (AAs) for simulated A/H3N2-like populations between the status quo (12-month forecast horizon and realistic submission delays) and realistic interventions.]
{Improvement of distances to the future (AAs) for simulated A/H3N2-like populations between the status quo (12-month forecast horizon and realistic submission delays) and realistic interventions.\
  The effects of interventions are the differences between distances to the future per future timepoint under the status quo and specific interventions.
...
%
\figsupp[Improvement of optimal distances to the future (AAs) for A/H3N2 populations between the status quo (12-month forecast horizon and realistic submission delays) and realistic interventions.]
{Improvement of optimal distances to the future (AAs) for A/H3N2 populations between the status quo (12-month forecast horizon and realistic submission delays) and realistic interventions.\
  \figsuppdata{Improvement of optimal distances to the future per future timepoint for A/H3N2 populations; see \url{https://doi.org/xxx}}}
{\includegraphics[width=6cm]{figures/h3n2_optimal_effects_of_realistic_interventions_on_distances_to_the_future}}\label{figsupp:h3n2_optimal_effects_of_realistic_interventions_on_distances_to_the_future}
%
\figsupp[Improvement of optimal distances to the future (AAs) for simulated A/H3N2-like populations between the status quo (12-month forecast horizon and realistic submission delays) and realistic interventions.]
{Improvement of optimal distances to the future (AAs) for simulated A/H3N2-like populations between the status quo (12-month forecast horizon and realistic submission delays) and realistic interventions.\
...
\begin{table}[htb]
  \begin{center}
    ...
    \caption{Improvement in A/H3N2 clade frequency forecast accuracy under realistic interventions of improved vaccine development (reducing 12-month to 6-month forecast horizon), improved surveillance (reducing submission delays from 3 months on average to 1 month), or a combination of both interventions.
      We measured improvements from the status quo (12-month forecast horizon and 3-month average submission delay) as the difference in total absolute clade frequency error per future timepoint and the number and proportion of future timepoints for which forecasts improved under the intervention.}
...
As expected, all interventions improved on the optimal distance to the future for both populations (\FIGSUPP[h3n2_effects_of_realistic_interventions]{h3n2_optimal_effects_of_realistic_interventions_on_distances_to_the_future} and \FIGSUPP[h3n2_effects_of_realistic_interventions]{simulated_optimal_effects_of_realistic_interventions_on_distances_to_the_future}).
...
These results confirmed the relatively stronger effect of reducing forecast horizons compared to submission delays.
They also confirmed that reducing submission delays can improve forecasts under optimal forecasting conditions.
...
Although decreasing submission delays only marginally improved long-term forecast accuracy, shorter delays increased the accuracy of current clade frequency estimates, reduced the bias toward underestimating current and future frequencies of larger clades, and improved forecasts 3 months into the future.
...
We expect that decreasing forecast horizons and submission delays will have similar effect sizes in other forecasting models, too.
...
From this work, we know that our current frequency estimates for large clades ($\ge$10\% frequency) under realistic submission delays have a wide range of errors (-16\% to 29\%).
Similarly, the range of 12-month forecast frequency errors under realistic delays include overestimates by up to 78\% and underestimates up to 78\%.
...
Of the 104,392 HA sequences in GISAID EpiFlu, 11,222 (11\%) were collected during this period with a mean submission delay of 98 days ($\sim$3 months) and a median delay of 74 days.
...
To estimate the effect of increased global sequencing capacity associated with the response to the SARS-CoV-2 pandemic, we summarized the delay distribution for sequences submitted to GISAID EpiFlu between January 1, 2022 and January 1, 2023.
...
We thank Katie Kistler and Marlin Figgins for their comments on early versions of this manuscript.
