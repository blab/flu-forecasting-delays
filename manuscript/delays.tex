...
These results show that reducing submission delays for natural H3N2 populations from a 3-month average to a 1-month average could reduce our uncertainty in current clade frequencies by half.
...
We also found that reducing submission delays from realistic to ideal conditions reduces the uncertainty in current clade frequencies by half.
...
We calculated the error in forecast frequencies for all clades with an initial frequency $\ge$10\% as the difference between predicted future frequencies under the given delay scenario and observed future frequencies without any delay.
...
This pattern matches our expectation that at any given initial timepoint the overestimation of one clade's future frequency must cause an underestimation of another current clade's future frequency.
However, we also observed a consistent bias toward the overestimation of future clade frequencies with median errors ranging from -1\% to -9\% across all forecast horizons and submission delays.
...
If we slightly underestimate current large clade frequencies, why do we slightly overestimate future large clade frequencies?
Are these the same clades or different clades?
One reason could be that we also underestimate current frequencies of small clades that quickly rise in frequency.
What correlates with higher negative forecast error? Initial clade frequency? Initial clade frequency error? Number of samples in the initial clade? Season?

There is a slight trend across all horizons and delays toward overestimating future frequencies of clades with higher initial estimated frequencies.
There is also a slight trend across all horizons and delays toward overestimating future frequencies of clades with overestimated initial frequencies.
In other words, we tend to overestimate the initial frequencies of some large clades and thereby overestimate their future frequencies, too.
At the same time, we slightly tend to underestimate large clade frequencies under realistic delays.
