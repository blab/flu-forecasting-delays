...
Vaccine update decisions depend on the diversity of currently circulating phylogenetic clades, groups of influenza viruses that all share a recent common ancestor.
...
Recent models include phenotypic data from serological experiments \citep{Morris2018,Huddleston2020,Meijers2023}, but these models still heavily rely on HA sequences to determine the diversity of clades circulating at the time of a forecast.
...
Even in the years immediately after the onset of the SARS-CoV-2 pandemic, we have observed a trend toward the reduced average submission delays that we would expect from increased global capacity for genome sequencing (\FIGSUPP[model_of_delays_and_horizons]{distribution_of_delays_by_pandemic_era}).
...
\caption{Model of submission delays and forecast horizons.
  A) Observed delays in days between collection of viral samples and submission of corresponding HA sequences to GISAID (purple) for samples collected in 2019 have a mean of 98 days (approximately 3 months).
  ...
  B) Long-term forecasting models historically predicted 12 months into the future from April and October because of the time required to develop and distribute a new vaccine \citep{Luksza:2014hj}.
  ...
  For each forecast horizon, we calculate the accuracy of forecasts under each of the three submission delays reflected above including no delay (blue), realistic delay (green), and ideal delay (orange).}\label{fig:model_of_delays_and_horizons}
%
...
\subsection{Reducing submission delays and forecast horizons decreases distances between predicted and observed future populations}
...
For each submission delay type and forecast horizon, we calculated the earth mover's distance between the predicted future populations under the given delay scenario and the observed future populations without any delay in sequence availability.
We anticipated that reducing either the submission delay or the forecast horizon would reduce the distance to the future in amino acids (AAs), representing increased accuracy of the forecasting models.

We found that reducing submission delays from a $\sim$3-month average delay in the realistic scenario to a $\sim$1-month average delay in the ideal scenario had relatively little effect on distance to the future.
...
In contrast, we found that reducing the forecast horizon from the current standard of 12 months linearly reduced the distance to the future population predicted by the LBI and mutational load model (\FIG{h3n2_distances_to_the_future}).
...
In this work, we showed that decreasing submission delays for seasonal influenza A/H3N2 HA sequences to public databases and decreasing the time to develop new vaccines improves our estimates of current and future populations, respectively.
...
We also confirmed that forecasts became more accurate and more precise with each 3-month reduction in forecast horizon from the status quo of 12 months.
...
We confirmed these effects with a previously validated forecasting model using both simulated and natural populations, but we expect that decreasing submission delays and forecast horizons will have similar effect sizes in other forecasting models, too.

Even without these recommended improvements to sequence submissions and vaccine development, these results inform important next steps to improve forecasting models.
...
Forecasts of future influenza populations represent a small part of the overall decision-making process for vaccine development compared to estimates of current populations.
Virologists measure antigenic drift in currently circulating clades with serological assays and use these experimental measurements to recommend potential vaccine candidates \citep{Morris2018,Loes2024}.
...
The appearance of even a single sequence in GISAID with a potentially important antigenic substitution could be enough to inform choices of vaccine candidate viruses.
