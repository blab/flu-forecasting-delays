...
For each combination of initial timepoint, final timepoint, and delay scenario, we identified all clades that were present at both timepoints, calculated initial and predicted future frequencies for the given delay, and calculated the corresponding observed future frequencies without delay.
...
Unlike distances to the future and like current clade frequency errors, forecast clade frequency errors appeared to be normally distributed (\FIG{h3n2_forecast_clade_frequency_errors}).
The 12-month forecast horizon produced the most extreme average forecast error and variance in error with a slight bias toward overestimation of future clade frequencies in all delay scenarios (\TABLE{h3n2_forecast_frequency_errors}).
As the forecast horizon decreased, the average error shifted toward a slight underestimation of future frequencies at the 3-month horizon.
...
The mean and median clade frequency errors were almost identical across all delay types (\TABLE{h3n2_forecast_frequency_errors}).
The same pattern held for absolute clade frequency errors, too (\TABLE{h3n2_absolute_forecast_frequency_errors}).
